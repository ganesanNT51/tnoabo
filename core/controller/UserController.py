...
# from core.model.LoginLog import LoginLog
...
@app.route('/encrypt_update', methods = ["GET", "POST"])
def encrypt_update():
    users =  User_details().get_all_users()
    data = []
    date_str = ""
    for u in users:
        date_str +=  str(u.get('user_id')) + "," + Cryptography.encrypt(u.get('user_id')) + "\n" 
...
#                     return redirect (url_for('user.Sample_lobby',user_id= encrypted_user_id)) 
    ...
    if request.method == "POST":
        ...
        if  not form_email:
            ...
            if user_output : 
                ...
                LoginLog().insertlog(user_id,form_email, datetime.now().strftime("%Y/%m/%d %H:%M:%S"))

                # commented by sridhar for testing
                # us.update_last_login(user_id,datetime.now());
...
@app.route('/lobby_screen' , methods = ["GET"])
def Lobby_screen():
    #  s = Sessions()
    c = CertModel()
    status = 1
    if session.get('user'):    
        ...
        return render_template('users/new_lobby_links.html', cert_count = cert_count,user_id = encrypted_user_id)
    else :
...
def PostHelpDesk():
    ...
    subject = "BOA 2021  - Help Desk feedback from  " "Dr."+full_name 
...
def BulkEmail():
    ...
    for user in users:
        ...
        user_id = user.user_id
        subject = "BOA 2021. First Annual Virtual Conference of BOA"
...
@app.route('/sample_lobby')
def Sample_lobby():
    return render_template('users/sample_lobby_page.html')  
    
@app.route('/login_logs')
def login_logs():
    data =  LoginLog().get_loginlogs()
    return render_template('login_logs.html',data=data)  
